...
conn.sendButton(m.chat, `_${command}_`.trim(), `*◈${name}*`, haha.data, [['🔄 𝚂𝙸𝙶𝚄𝙸𝙴𝙽𝚃𝙴 🔄', `/${command}`]], fgif)}
if (command == 'pubg') {
...
let json = res.data
let url = json.url
conn.sendButton(m.chat, `_${command}_`.trim(), `*◈•${name}*`, url, [['Gracias💖', `/bot gracias`]], fgif)}
if (command == 'coffee') {
